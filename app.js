...
function inicializarBuscaPlantasModal() {
  ...
  inputBusca.addEventListener('input', async (e) => {
    clearTimeout(timeoutBuscaModalPlanta);
    const valor = e.target.value.trim();
    
    if (valor.length < 2) {
      dropdown.style.display = 'none';
      document.getElementById('mInfoPlanta').style.display = 'none';
      return;
    }
    ...
    timeoutBuscaModalPlanta = setTimeout(async () => {
      try {
        ...
        if (!data.plantas || data.plantas.length === 0) {
          dropdown.innerHTML = '<div class="dropdown-vazio">Nenhuma planta encontrada</div>';
          dropdown.style.display = 'block';
          return;
        }
        
        dropdown.innerHTML = data.plantas.map((planta) => `
          <div class="dropdown-item" onclick="selecionarPlantaModal(${JSON.stringify(planta).replace(/"/g, '&quot;')})">
            <div class="dropdown-item-nome">🌿 ${planta.nome_pt || planta.nome_en}</div>
            <div class="dropdown-item-cientifico">${planta.nome_en}</div>
          </div>
        `).join('');
        ...
        dropdown.innerHTML = '<div class="dropdown-vazio">Erro ao conectar com a API</div>';
        dropdown.style.display = 'block';
      }
    }, 300);
  });
  ...
  document.addEventListener('click', (e) => {
    if (e.target !== inputBusca && e.target !== dropdown) {
      dropdown.style.display = 'none';
    }
  });
}

function selecionarPlantaModal(planta) {
  document.getElementById('mNome').value = planta.nome_pt || planta.nome_en;
  document.getElementById('mEspecie').value = planta.nome_en;
  
  if (planta.agua_minima) {
    document.getElementById('mMin').value = planta.agua_minima;
  }
  if (planta.agua_maxima) {
    document.getElementById('mMax').value = planta.agua_maxima;
  }
  
  document.getElementById('mDropdown').style.display = 'none';
  document.getElementById('mBuscaPlanta').value = planta.nome_pt || planta.nome_en;
  
  const infoPlanta = document.getElementById('mInfoPlanta');
  document.getElementById('mInfoNome').textContent = planta.nome_pt || planta.nome_en;
  document.getElementById('mInfoCientifico').textContent = planta.nome_en;
  
  let regaInfo = '';
  if (planta.agua_minima || planta.agua_maxima) {
    regaInfo = `💧 Rega sugerida: ${planta.agua_minima || '?'} - ${planta.agua_maxima || '?'} dias`;
  }
  document.getElementById('mInfoRega').textContent = regaInfo;
  
  infoPlanta.style.display = 'block';
}

// ═══════════════════════════════════════════════════════
// PLANTAS - FUNÇÕES ORIGINAIS
// ═══════════════════════════════════════════════════════
...
function abrirModalPlanta() {
  ["mNome","mEspecie", "mBuscaPlanta"].forEach(id=>document.getElementById(id).value="");
